...
install_chrome_version() {
    ...
    for i in {1..5}; do
        if sudo curl -sS --continue-at - "https://fs.catchpoint.com/UtilFiles/linux_chrome/catchpoint_chrome_linux_$version.7z" | pv -s $size -b -p -t -e -r | sudo tee /tmp/catchpoint_chrome_linux_$version.7z > /dev/null; then
            download_success=1
            break
        else
    ...
    for i in {1..5}; do
        if cd /tmp/ && sudo 7za x "/tmp/catchpoint_chrome_linux_$version.7z" > /dev/null; then
            ...
            if sudo wget "https://fs.catchpoint.com/UtilFiles/linux_chrome/catchpoint_chrome_linux_$version.7z" -O /tmp/catchpoint_chrome_linux_$version.7z; then
                download_success=1
            else
...
sudo rm /var/tmp/fs_allchromedeploy.sh 2> /dev/null 